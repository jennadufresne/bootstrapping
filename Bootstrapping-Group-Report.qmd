...
contendors for statistical modeling using the Bootstrap method.

### Statistical Modeling

We will look at two applications of Bootstrapping, both using the Monte Carlo 
method (new samples are the exact size as the original sample). The first 
application is estimating the population mean and standard deviation of the 
cardiovascular diseases death rate. Second, we will test a simple linear 
regression that estimate the number of interpersonal violence deaths on the 
population of a country for variability of the model.

In both experiments we will need to take a sample of our population data to
continue with the Bootstrap methods. We will take a sample of the population,
without replacement, with size $n = 20$.
...
Here we are going to be estimating the population mean and standard deviation 
using the Bootstrap method. We can use Bootstrap case re-sampling to find 1,000 
new samples using replacement. For each new sample created we will save the mean 
...
Now that we have the mean and standard deviations for all 1,000 samples creating 
using Bootstrap re-sampling, we can trim the top and bottom 2.5% to find the 95% 
confidence interval for the population mean and standard deviation.
...
deviation vectors we will find our 95% estimated population intervals. The 95% 
...
```{r}
# Display a histogram of the cardiovascular diseases deaths estimated 
# population means rate
ggplot() +
...
model. We will use the initial sample created in the first experiment as the 
initial sample for this experiment as well. For the model, we will see if the 
population of a country can be used to predict the deaths caused by 
interpersonal violence in 2019. Again, we will create 1,000 new samples. For 
each one of the new samples we will fit a simple linear regression model and 
save the coefficient and regression parameter.
...
Now we can compare the full population model, sample model, and Bootstrapping 
